...
The gateway uses the same Aspire service defaults as the API. Requests for the host page, static
assets, and proxied `/api` routes therefore appear as gateway server spans. API requests carry the
distributed trace context onward, so Aspire can show the browser-facing gateway request and API
work in one trace.

The WebAssembly runtime executes inside the browser, not inside the Aspire-managed Web development
server. Component rendering, client-side navigation, and UI events therefore do not automatically
produce Aspire spans. Capturing those semantic operations requires deliberate browser-side
OpenTelemetry instrumentation and a suitable telemetry intake; separating the Web resource
improves server-boundary visibility but does not itself instrument browser execution.
