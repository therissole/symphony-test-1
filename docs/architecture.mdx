...
The MudBlazor client is a standalone Blazor WebAssembly project referenced by the API only for
static-asset hosting. The dependency points from host to client: the client does not reference the
API assembly, persistence packages, or server DTOs. It calls relative `/api` routes over
`HttpClient`, so development and deployment remain same-origin without CORS or separate endpoint
configuration.
...
tests/integration/Infrastructure hosted WASM route behavior
...
deployed WASM UI through every CRUD operation against a Testcontainers PostgreSQL database.
