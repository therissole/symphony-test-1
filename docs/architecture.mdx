...
| --- | --- |
| `NpgsqlDataSource` registration | Connection pooling and lifetime are platform mechanics |
| Global Problem Details | Unexpected failures need one safe response policy |
| Serilog request logging | Applies uniformly to the HTTP pipeline |
| OpenTelemetry setup | Captures platform-level request and database spans |
...
Database migrations remain centralized because the sample uses one relational schema. A larger
modular system could own migrations per module without changing the slice design.
